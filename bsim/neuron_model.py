...
class NeuronModel(BaseModel):
    ...
    def _compile_h(self):
        ...
        h_file.write("__global void update_%s_gpu(%s *neuron_data, int num, int start_id);\n" %
                     (self.name.lower(), self.name))
        h_file.write("\n")
    ...
    def _compile_py(self):
        ...
        if self.refract:
            py_file.write('("refract_step", POINTER(c_int),\n')

        for i in self.parameters['variable'][:-1]:
            py_file.write('("%s"), POINTER(c_float),\n'% i)
    ...
    def _compile_cpp(self):
        ...
        if self.refract:
            c_file.write('\t\tgpu->refract_step = copyToGPU<real>(cpu->p_refaract_step, num);\n')
    ...
    def _compile_cu(self):
        ...
        cu_file.write("__global void find_%s_gpu(%s *neuron_data, int num, int start_id)\n" %
                      (self.name.lower(), self.name))
        cu_file.write("{\n")
        ...
        cu_file.write("\t\t\n")
        cu_file.write("\t\tbool actived = d_neurons->p_refrac_step[idx] <= 0;\n")
        cu_file.write("\t\t\n")
        ...
        cu_file.write("\t\t\td_neurons->p_refrac_step[idx] = d_neurons->p_refrac_step[idx] - 1;\n")
        cu_file.write("\t\t}\n")
        cu_file.write("\t\t__syncthreads();\n")
        cu_file.write("\t\t\n")
        ...
        cu_file.write("__global void update_%s_gpu(%s *neuron_data, int num, int start_id)\n" %
                      (self.name.lower(), self.name))
        cu_file.write("{\n")
        ...
        cu_file.write("\t\td_neurons->p_vm[nid] = d_neurons->p_Cm[nid] * d_neurons->p_vm[nid] + d_neurons->p_v_tmp[nid] + d_neurons->p_i_E[nid] * d_neurons->p_C_E[nid] + d_neurons->p_i_I[nid] * d_neurons->p_C_I[nid];\n")
        cu_file.write("\t\t\n")
        cu_file.write("\t\td_neurons->p_i_E[nid] *= d_neurons->p_CE[nid];\n")
        cu_file.write("\t\td_neurons->p_i_I[nid] *= d_neurons->p_CI[nid];\n")
        cu_file.write("\t\t\n")
        cu_file.write("\t\tfired = d_neurons->p_vm[nid] >= d_neurons->p_v_thresh[nid];\n")
        cu_file.write("\t\t\n")
        ...
        cu_file.write("\t\t\t}\n")
        cu_file.write("\t\t\t\n")
        cu_file.write("\t\t\td_neurons->p_refrac_step[nid] = d_neurons->p_refrac_time[nid] - 1;\n")
        cu_file.write("\t\t\td_neurons->p_vm[nid] = d_neurons->p_v_reset[nid];\n")
        cu_file.write("\t\t} else {\n")
        cu_file.write("\t\t\tgXInput[gnid] += gNeuronInput[gnid] + gNeuronInput_I[gnid];\n")
        cu_file.write("\t\t\td_neurons->p_i_E[nid] += gNeuronInput[gnid];\n")
        cu_file.write("\t\t\td_neurons->p_i_I[nid] += gNeuronInput_I[gnid];\n")
        cu_file.write("\t\t}\n")
        cu_file.write("\t\t\n")
