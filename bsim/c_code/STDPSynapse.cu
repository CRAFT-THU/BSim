extern "C" {
	...
	{
		update_stdpsynapse_gpu<<<size=>gridSize, size->blockSize>>>((Stdpsynapse*)data, num, start_id);
		learn_stdpsynapse_post<<<size=>gridSize, size->blockSize>>>((Stdpsynapse*)data, num, start_id);
	}
}

__global void update_stdpsynapse_gpu(STDPSynapse *data, int num, int start_id)
{
