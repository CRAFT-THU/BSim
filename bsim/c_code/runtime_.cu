...
__global__ void add_cross_neuron(int *ids, int num, int t)
{
	...
	if (tid == 0) {
		g_fired_tableSizes[delay_idx] += num;
	}
}
...
__global__ void deliver_neurons(int *idx2index, int *crossnode_index2idx, int *global_cross_data, int *fired_n_num, int node_num)
{
	...
	int fired_size = g_fired_tableSizes[gCurrentIdx];
	for (int node = 0; node < node_num; node++) {
		for (int idx = tid; idx < fired_size; idx += blockDim.x * gridDim.x) {
			int nid = g_fired_table[g_fired_tableCap*gCurrentIdx + idx];
			...
			if (cross_cnt > 0) {
				commit2globalTable(cross_neuron_id, cross_cnt, global_cross_data, &fired_n_num[node], g_fired_tableCap*node);
				if (threadIdx.x == 0) {
					cross_cnt = 0;
				}
			}
