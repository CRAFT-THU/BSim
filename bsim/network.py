...
class Network(object):
    ...
    def _compile_connection(self):
        ...
        for i in self.connection_data:
            self.connection_data.delay_start = [0] * (length)
            self.connection_data.delay_num = [0] * (length)
                ...
                for i in range(len(self.neuron2synapses[t])):
                    ...
                    for j in range(len(self.neuron2synapses[i][t])):
                        if self.neuron2synapses[i][t][j].parameters['delay'] == d:
                            if count == 0:
                                self.connection_data[t].delay_start[i + d * self.neuron_num] = self.synapse2id[self.synapse2id(self.neuron2synapses[t][i][j])]
    ...
    def compile_(self):
        ...
        # TODO compile the network data structure
