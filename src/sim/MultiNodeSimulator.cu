...
int MultiNodeSimulator::run(real time, FireInfo &log)
{
	...
	pthread_barrier_init(&nodeCycleBarrier, NULL, device_count);

	MultiNetwork multiNet(network, device_count);
	DistriNetwork *node_nets = multiNet.buildNetworks();
	assert(node_nets != NULL);
	gCrossDataNode = multiNet.arrangeCrossNodeDataGPU(device_count);
	assert(gCrossDataNode != NULL);
	...
	for (int i=0; i<device_count; i++) {
		...
		int ret = pthread_create(&(thread_ids[i]), NULL, &run_thread_node, (void*)&(node_nets[i]));
		assert(ret == 0);
	}
	...
	pthread_barrier_destroy(&nodeCycleBarrier);
...
void * run_thread_node(void *para) {
	...
	for (int time=0; time<network->_sim_cycle; time++) {
		...
		pthread_barrier_wait(&nodeCycleBarrier);
		...
		checkCudaErrors(cudaMemcpy(gCrossDataNode->_fired_num + network->_node_idx * network->_node_num, c_g_fired_n_num, sizeof(int)*network->_node_num, cudaMemcpyDeviceToHost));
		//gettimeofday(&t3, NULL);

		for (int i=0; i< network->_node_num; i++) {
			int idx2i = network->_node_idx * network->_node_num + i;
			assert(gCrossDataNode->_fired_num[idx2i] <= gCrossDataNode->_max_num[idx2i]);
			if (gCrossDataNode->_fired_num[idx2i] > 0) {
				checkCudaErrors(cudaMemcpyPeer(gCrossDataNode->_fired_arrays[idx2i], i, c_g_global_cross_data + allNeuronNum * i, network->_node_idx, gCrossDataNode->_fired_num[idx2i] * sizeof(int)));
			}
		}
		...
		for (int i=0; i< network->_node_num; i++) {
			...
			if (gCrossDataNode->_fired_num[i2idx] > 0) {
				addCrossNeurons(gCrossDataNode->_fired_arrays[i2idx], gCrossDataNode->_fired_num[i2idx], time);
			}
		}
