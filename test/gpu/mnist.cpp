...
int main(int argc, char **argv)
{
	...
	for (int batch = 0; batch < dataset.test_images.size(); batch++) {
		...
		free(fire_info["count"].data);
		fire_info["count"].data = NULL;
		fire_info["count"].size = 0;
		rate = NULL;
		...
		if (batch % 100 == 99) {
			printf("%d: %lf\n", batch, (double)matched_batches/(batch+1));
		}
	}

	printf("%lf\n", (double)matched_batches/(dataset.test_images.size()));
