...
{
	void *ret = NULL;
	checkCudaErrors(cudaMalloc((void**)&(ret), sizeof(GLIFNeuron)*1));
	checkCudaErrors(cudaMemset(ret, 0, sizeof(GLIFNeuron)*1));
	return ret;
}
...
void *cudaAllocLIF(void *pCPU, int num)
{
	...
	checkCudaErrors(cudaMemcpy(ret, tmp, sizeof(GLIFNeuron)*1, cudaMemcpyHostToDevice));
	free(tmp);
	tmp = NULL;
	return ret;
}
...
void *cudaAllocLIFPara(void *pCPU, int num)
{
	...
	GLIFNeuron *ret = (GLIFNeuron*)malloc(sizeof(GLIFNeuron)*1);
	memset(ret, 0, sizeof(GLIFNeuron)*1);
...
{
	GLIFNeuron *pTmp = (GLIFNeuron*)malloc(sizeof(GLIFNeuron)*1);
	memset(pTmp, 0, sizeof(GLIFNeuron)*1);
	checkCudaErrors(cudaMemcpy(pTmp, pGPU, sizeof(GLIFNeuron)*1, cudaMemcpyDeviceToHost));
...
{
	GLIFNeuron *pC = (GLIFNeuron*)pCPU;
	GLIFNeuron *pG = (GLIFNeuron*)pGPU;
...
{
	GLIFNeuron *tmp = (GLIFNeuron*)malloc(sizeof(GLIFNeuron)*1);
	memset(tmp, 0, sizeof(GLIFNeuron)*1);
	checkCudaErrors(cudaMemcpy(tmp, pGPU, sizeof(GLIFNeuron)*1, cudaMemcpyDeviceToHost));
	cudaFreeLIFPara(tmp);
	free(tmp);
	tmp = NULL;
	cudaFree(pGPU);
