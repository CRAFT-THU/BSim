...
{
	GLIFNeurons *pTmp = (GLIFNeurons*)malloc(sizeof(GLIFNeurons)*1);
	memset(pTmp, 0, sizeof(GLIFNeurons)*1);
	checkCudaErrors(cudaMemcpy(pTmp, pGPU, sizeof(GLIFNeurons)*1, cudaMemcpyDeviceToHost));
